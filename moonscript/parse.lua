...
local White, Break, Stop, LitmoonCommentLine, AlphaNum, Num, Shebang, Comment, Space, SomeSpace, SpaceBreak, mkEmptyLine, _Name
do
  local _obj_0 = require("moonscript.parse.literals")
  White, Break, Stop, LitmoonCommentLine, AlphaNum, Num, Shebang, Comment, Space, SomeSpace, SpaceBreak, mkEmptyLine, _Name = _obj_0.White, _obj_0.Break, _obj_0.Stop, _obj_0.LitmoonCommentLine, _obj_0.AlphaNum, _obj_0.Num, _obj_0.Shebang, _obj_0.Comment, _obj_0.Space, _obj_0.SomeSpace, _obj_0.SpaceBreak, _obj_0.mkEmptyLine, _obj_0.Name
end
...
local build_grammar_factory
build_grammar_factory = function(litmoon)
  if litmoon == nil then
    litmoon = false
  end
  local EmptyLine = mkEmptyLine(litmoon)
  return wrap_env(debug_grammar, function(root)
    local _indent = Stack((litmoon and 4 or 0))
...
local build_grammar = build_grammar_factory()
local file_parser
file_parser = function(litmoon)
  if litmoon == nil then
    litmoon = false
  end
  local g, state
...
return {
  ...
  string = function(str, litmoon)
    if litmoon == nil then
      litmoon = false
    end
    return file_parser(litmoon):match(str)
  end
}
